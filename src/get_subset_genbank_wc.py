...
def make_files_with_id(taxonid, DB,outfilen,outfile_tbln,remove_genomes=False, limitlist = None):
    ...
    while len(stack) > 0:
        ...
        if str(id) in taxonids:
            continue
        c.execute("select name,custom_id,custom_parent_id,custom_name from taxonomy where ncbi_id = ? and name_class = 'scientific name'",(id,))
        l = c.fetchall()
        for j in l:
            tname = str(j[0])
            if j[1] != None:
                ...
                if outfilen != None and outfile_tbln != None:
                    if str(j[1]) in keepers:
                        if len(str(j[7])) > 10000:
                            ...
                            outfile.write(seqst+"\n")
                    outfile_tbl.write(str(j[0])+"\t"+str(j[1])+"\t"+str(j[2])+"\t"+str(j[3])+"\t"+str(tname)+"\t"+str(j[5])+"\t"+str(j[6])+"\t"+str(custom_id)+"\t"+str(custom_parent_id)+"\t"+str(custom_name)+"\n")
                # we are returning
                else:
...
def make_files_with_id_justtable(taxonid, DB,outfile_tbln):
    ...
    while len(stack) > 0:
        ...
        if custom_id != None: 
            ...
            l = c.fetchall()
            for j in l:
                tbls = str(j[0])+"\t"+str(j[1])+"\t"+str(j[2])+"\t"+str(j[3])+"\t"+str(tname)+"\t"+str(j[5])+"\t"+str(j[6])+"\t"+str(custom_id)+"\t"+str(custom_parent_id)+"\t"+str(custom_name)
                if outfile_tbln != None:
