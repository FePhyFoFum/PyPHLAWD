...
def generate_argparser():
    parser = ap.ArgumentParser(prog="find_good_clusters_for_concat_wc.py",
...
if __name__ == "__main__":
    ...
    if rename == 'y' or rename == '#':
        ...
        rtn = cld.split("/")[-1]
        cmd = py+" "+DI+"change_id_to_ncbi_fasta_mult_wc.py "+tab+" "+ " ".join(keeps)
        #print cmd
        os.system(cmd)
        ...
        if constraint == 'y':
            ...
            if len(dbname) > 2 and len(baseid) > 2:
                cmd = py+" "+DI+"make_constraint_from_ncbialn_wc.py "+dbname+" "+baseid+" "+cld+"/"+rtn+"_outaln > "+cld+"/"+rtn+"_outaln.constraint.tre"
                os.system(cmd)
